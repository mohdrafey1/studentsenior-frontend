...
const CommunityPage = () => {
  ...
  const likeComment = async (postId, commentId) => {
    if (!likedComments.includes(commentId)) {
      try {
        await apiRequest(`${url}/${postId}/comments/${commentId}/like`, 'POST');
        dispatch(fetchPosts(collegeId));
    ...
    <div className="container bg-gradient-to-t from-sky-200 to-white min-h-screen min-w-full">
      ...
      <div className="max-w-7xl mx-auto p-5">
        ...
        <p className="italic text-center text-xs sm:text-base">
          "Connect, share, and ask your questions and doubts through the
          community."
        </p>
        <Seo
          ...
          <div className="flex flex-wrap gap-4 justify-center">
            {posts.map((post) => (
              <PostCard
                ...
                onShowComments={() => handleShowComments(post._id)}
                onAddComment={() => handleAddComment(post._id)}
                onLikeComment={(commentId) => likeComment(post._id, commentId)}
          ...
          footer={
            <div className="flex py-4 gap-3 lg:justify-end justify-center">
              ...
              <button
                ...
                disabled={hookLoadingStates.deletePost[postIdToDelete]}
              >
        ...
        {latestComment && (
          <ul>
            <li key={latestComment._id}>
              <p className="line-clamp-1">{latestComment.content}</p>
              <div className="flex items-center justify-between mt-2">
                <button
                  className={`text-blue-500 ${
                    likedComments.includes(latestComment._id)
                      ? 'opacity-50 cursor-not-allowed'
                      : ''
                  }`}
                  onClick={() => onLikeComment(latestComment._id)}
                  disabled={likedComments.includes(latestComment._id)}
                >
                  <i className="fa-regular fa-heart"></i> ({latestComment.likes}
                  )
                </button>
                {latestComment.author._id === ownerId && (
                  <button
                    className="text-red-500"
                    onClick={() => onDeleteComment(latestComment._id)}
                    disabled={hookLoadingStates.deleteComment[post._id]}
                  >
                    {hookLoadingStates.deleteComment[post._id] ? (
                      <i className="fa fa-spinner fa-spin"></i>
                    ) : (
