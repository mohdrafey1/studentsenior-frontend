...
>>>>>>> 81caa9540474d85015bef0d185d0a79b7f7e7782
  ...
  const verifyPayment = async () => {
    try {
      if (!transactionId || !Object.values(PAYMENT_STATUS).includes(status)) {
        throw new Error('Invalid payment parameters');
      }

      const res = await fetch(`${API_BASE_URL}/api/phonepe/${transactionId}`, {
        credentials: 'include',
      });
        ...
        <div className="text-center mb-8">
          ...
          {paymentStatus === 'paid' && (
            <>
              ...
              <p className="text-gray-600 dark:text-gray-300">
                {['note_purchase', 'pyq_purchase', 'add_points'].includes(
                  paymentDetails?.typeOfPurchase,
                ) && paymentDetails?.redirectBackUrl ? (
                  <>
                    Thank you for your purchase. You'll be redirected back
                    shortly.
                    <p className="text-sm text-gray-500 mt-2">
                      Not redirecting?{' '}
                      <a
                        href={paymentDetails.redirectBackUrl}
          ...
          {paymentStatus === 'failed' && (
            <>
              ...
              <p className="text-gray-600 text-xs dark:text-gray-300">
                If your payment is deducted and does not return within 2 days
                please contact us
              </p>
            </>
          )}
          ...
          {paymentStatus === 'pending' && (
            <>
              ...
              <p className="text-gray-600 dark:text-gray-300">
                Your payment is still being processed. Please wait or retry
                later.
              </p>
              <p className="text-gray-600 text-xs dark:text-gray-300">
                If your payment is deducted and does not return within 2 days
        ...
        <div className="mt-8 flex flex-col sm:flex-row justify-between gap-4">
          ...
          {paymentStatus === 'pending' &&
            !['note_purchase', 'pyq_purchase', 'add_points'].includes(
              ...
              >
                <i className="fa-solid fa-hourglass-half"></i> Retry Payment
              </button>
            )}
          ...
          {paymentStatus === 'pending' &&
            ...
            paymentDetails?.redirectBackUrl && (
              <button
                onClick={() => {
                  window.location.href = paymentDetails.redirectBackUrl;
                }}
                className="flex-1 flex items-center justify-center gap-2 px-6 py-3 border border-transparent rounded-md shadow-sm text-base font-medium text-black bg-yellow-500 hover:bg-yellow-600"
              >
                <i className="fa-solid fa-hourglass-half"></i> Try Again
              </button>
            )}
          {paymentStatus === 'failed' &&
            ['note_purchase', 'pyq_purchase', 'add_points'].includes(
