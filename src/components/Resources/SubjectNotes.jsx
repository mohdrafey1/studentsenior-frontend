...
function SubjectNotes() {
  ...
  const likeNotes = async (noteId) => {
    try {
      ...
      toast.success(response.message || 'Note liked/unliked successfully.');
    } catch (err) {
  ...
  const handleUpdatenote = async () => {
    try {
      setSubmitting(true);
      const response = await fetch(`${api.subjectNotes}/${editNoteData._id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  if (notesError) {
    return (
      <div className="h-screen flex justify-center items-center">
        <div>
          <p className="text-center text-red-500 mb-4">{notesError}</p>
          <Link
            ...
            <div className="items-center mb-4 text-white">
              ...
              <div className="text-gray-900 dark:text-white">
                ...
                <ul className="list-disc ml-4 mt-2">
                  ...
                  <li>You can upload 1 unit or the whole in one upload</li>
                  <li>
                    Rewards will be given only after the notes are approved
                  </li>
                  <li>Duplicate and invalid notes are not allowed</li>
                  <li>
                    Labs will be approved only once unless they are better than
        ...
        {subjectNotes.length > 0 ? (
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <button
                    ...
                    className={`text-lg transition-colors duration-200 ${
                      Array.isArray(note.likes) && note.likes.includes(ownerId)
                        ? 'text-red-500'
          ...
          <div className="h-screen text-center text-gray-600 col-span-full flex items-center justify-center">
            <p className="text-xl font-semibold">
              No notes available for this subject. Please add if you have any.
            </p>
          </div>
        )}
      </div>
