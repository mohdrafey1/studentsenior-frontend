        ...
        return (
          ...
          >
            ...
            <div className="text-sm text-gray-600 mb-4 space-y-1">
              ...
              <p>
                <strong>Branch:</strong> {note.subject?.branch?.branchCode}
              </p>
            </div>
...
=======
    if (notes.length === 0) {
        return (
            <div className="text-center py-16 bg-gray-50 rounded-xl border border-gray-100">
                ...
                <h3 className="mt-6 text-xl font-medium text-gray-900">No notes available</h3>
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-5 gap-4 lg:gap-6">
            {notes.map((note) => {
                const courseCode = note.subject?.branch?.course?.courseCode.toLowerCase();
                const branchCode = note.subject?.branch?.branchCode.toLowerCase();
                const subjectCode = note.subject?.subjectCode.toLowerCase();
                const noteUrl = `/${collegeName}/resources/${courseCode}/${branchCode}/notes/${subjectCode}/${note.slug}`;
                ...
                return (
                    <div
                        key={note._id}
                        className={`bg-white rounded-xl border ${isHovered ? 'border-blue-200 shadow-lg' : 'border-gray-100 shadow-sm'} overflow-hidden transition-all duration-300 flex flex-col h-full group relative`}
                        onClick={(e) => handleCardClick(noteUrl, e)}
                    ...
                    >
                        {/* Highlight Accent */}
                        <div className={`h-1 w-full ${note.isPaid ? 'bg-gradient-to-r from-purple-500 to-indigo-500' : 'bg-blue-500'}`}></div>
                        ...
                        <div className="p-4 border-b border-gray-100">
                            <div className="flex justify-between items-start">
                                ...
                                {note.isPaid && (
                                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-800 ml-2 flex-shrink-0">
                        ...
                        <div className="p-4 flex-grow">
                            <div className="grid grid-cols-2 gap-3 text-sm">
                                <div className="flex flex-col">
                                    ...
                                    <span className="font-medium text-gray-800">{note.subject?.subjectCode || 'N/A'}</span>
                                </div>
                                <div className="flex flex-col">
                                    <span className="text-gray-500 mb-1">Semester</span>
                                    <span className="font-medium text-gray-800">{note.subject?.semester || 'N/A'}</span>
                                </div>
                                <div className="flex flex-col col-span-2">
                                    <span className="text-gray-500 mb-1">Branch</span>
                                    <span className="font-medium text-gray-800">{note.subject?.branch?.branchCode || 'N/A'}</span>
                                </div>
                            </div>
                        </div>

                        {/* Card Footer */}
                        <div className="px-4 py-3 bg-gray-50 mt-auto border-t border-gray-100">
                            {isAuthenticated ? (
                                <Link
                                    to={noteUrl}
                                    className={`w-full flex items-center justify-center py-2 px-3 rounded-lg text-sm font-medium transition-all ${note.isPaid
                                ...
                                >
                                    {note.isPaid ? (
                                        <>
                                            ...
                                            <span className="whitespace-nowrap">View Premium Note</span>
                                        </>
                                    ) : (
                                        <>
                                            <svg className="w-4 h-4 mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                            ...
                                            <span className="whitespace-nowrap">View Note</span>
                                        </>
                                    )}
                                </Link>
                            ) : (
