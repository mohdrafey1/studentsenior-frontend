            ...
            <div className="py-12 px-4">
                ...
                <div className="text-center mb-8">
                    ...
                    <p className="text-gray-600 text-base sm:text-lg mt-2">
                        Explore the impact we've made with our resources, community, and products!
                    </p>
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-6 max-w-5xl mx-auto">
                    {achievements.map((item, index) => (
                        <div
                            key={item.id}
                            className={`${index % 2 === 0 ? 'bg-sky-200' : 'bg-red-200'
                        ...
                        >
                            {/* Icon */}
                            <div
                                className="absolute bottom-[-100px] left-1/2 transform -translate-x-1/2 lg:group-hover:translate-y-[-100%] transition-all duration-200 ease-out"
                            >
                                <img
                                    src={item.icon}
                                    alt={item.title}
                            ...
                            <div className="relative z-10 lg:group-hover:-translate-y-8 transition-transform duration-200 ease-out flex flex-col justify-center h-full">
                                <h3 className="lg:text-4xl text-3xl font-extrabold text-gray-800">{currentCounts[index]}+</h3>
                                <p className="text-base sm:text-lg font-semibold text-gray-600 mt-2">{item.title}</p>
                            </div>
                        </div>
                    ))}
                </div>
